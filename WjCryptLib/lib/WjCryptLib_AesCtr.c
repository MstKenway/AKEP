...
AesCtrXor
    ...
    XorBuffers(InBuffer, Context->CurrentCipherBlock + (AES_BLOCK_SIZE - amountAvailableInBlock), OutBuffer,
               firstChunkSize);
...
#pragma omp parallel for firstprivate( preCipherBlock, cipherBlockIndex ) lastprivate( encCipherBlock, cipherBlockIndex )
    ...
    for (i = 0; i < numIterations; i++) {
        ...
        XorBuffers((uint8_t *) InBuffer + outputOffset, encCipherBlock, (uint8_t *) OutBuffer + outputOffset,
                   chunkSize);
    }
